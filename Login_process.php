...
if (isset($_POST['nombre_usuario']) && isset($_POST['correo']) && isset($_POST['contrasena'])) {
    ...
    $sql = "SELECT UsuarioID, NombreUsuario, ContrasenaHash, Rol FROM viv_perfilesusuario WHERE NombreUsuario = ? AND CorreoElectronico = ?";
    ...
    if ($stmt->num_rows === 1) {
        // Asegurarse de extraer todos los valores correctos
        $stmt->bind_result($UsuarioIDBD, $NombreUsuarioBD, $ContrasenaHashBD, $Rol);
        $stmt->fetch();
        ...
        if (password_verify($Contrasena, $ContrasenaHashBD)) {
            ...
            $_SESSION['UsuarioID'] = $UsuarioIDBD; // Almacenar el UsuarioID correctamente en la sesión
            $_SESSION['rol'] = $Rol;
