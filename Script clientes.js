document.addEventListener('DOMContentLoaded', () => {
    ...
    botonConfirmarCompra.addEventListener('click', () => {
        ...
        if (carrito.length > 0) {
            // Obtén el UsuarioID de la sesión (esto depende de cómo se maneje la sesión en tu aplicación)
            const usuarioID = <?php echo $_SESSION['UsuarioID']; ?>;

            // Enviar la solicitud AJAX con los datos del carrito y el UsuarioID
            fetch('Registro pedido.php', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    window.location.href = 'ConfirmacionCompra.php'; // Redirigir a la página de confirmación
                } else {
